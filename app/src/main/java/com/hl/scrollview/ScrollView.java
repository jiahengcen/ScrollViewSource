 ...
 * Android 6.0
 * ScrollView
 * 这只适用于分析功能过程。并不适用于做为控件。
 * 在computeScroll中overScrollBy（API 9)
 * SCROLL （滚动） 表示手指没有离开屏幕，onTouch的MOVE事件一直在被触发
 * FLING  （抛动)  表示手指已经离开屏幕，页面在惯性的作用下继续运动的效果
 *
 * 在SCROLL情况是onTouchEvent一直在触发MOVE事件。MOVE事件中overScrollBy
 */
...
public class ScrollView extends FrameLayout {
    ...
    public ScrollView(Context context, AttributeSet attrs) {
        this(context, attrs, Resources.getSystem().getIdentifier("scrollViewStyle", "values", "android"));
...
//                    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //Log.e("HLA", "Move ev.getY():" + ev.getY() + "ev.getRawY()" + ev.getRawY());
    ...
    protected void onOverScrolled(int scrollX, int scrollY,
        ...
        // Log.e("HLA","onOverScrolled");
        // Treat animating scrolls differently; see #computeScroll() for why.
        if (!mScroller.isFinished()) {
            final int oldX = getScrollX();
            final int oldY = getScrollY();
            //在Fling状态下通过setScrollY进行
            ...
            if (clampedY) {
                //当子视图被拉扯超过了上下边界，会发生回滚动画。我并没有找到发生这件事的可能的情况
                //解释一下子视图被拉扯超过了上下边界，比如我们手指向下滑动，当子视图已经到达了顶部，如果我们子视图还会向下走一部分，当我们松开手会发生回滚，子视图重新顶到头部。对于底部同样的效果
                //现在Fling到底部还是顶部都是使用发光的EdgeEffect，并不会触发这里。
                mScroller.springBack(getScrollX(), getScrollY(), 0, 0, 0, getScrollRange());
            }
        } else {
           // Log.e("HLA","onOverScrolled scrollTo");
            //在Scroll状态下滚动
...
//                }
    ...
    public void computeScroll() {
        //Log.e("HLA", "computeScroll");
        //只有在fling的时候mScroller.computeScrollOffset()=true
        if (mScroller.computeScrollOffset()) {
            ...
            Log.e("HLA", "computeScroll oldY:" + oldY + " getCurrY:" + y);
            if (oldX != x || oldY != y) {
                final int range = getScrollRange();
                final int overscrollMode = getOverScrollMode();
...
//    }
    ...
    protected float getVerticalScrollFactor() {
        if (mVerticalScrollFactor == 0) {
            mVerticalScrollFactor = (int) (64 * getContext().getResources().getDisplayMetrics().density + 0.5f);
        }
